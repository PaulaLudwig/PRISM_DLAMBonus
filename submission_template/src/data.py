...
def load_panel(data_dir):
    # private evaluation ships one file: known history (with real target)
    # plus known future covariates, target left blank for rows to predict
    frame = pd.read_csv(data_dir / "test_input.csv", parse_dates=[TIME_COL])
    frame = frame.sort_values([SERIES_COL, TIME_COL], kind="mergesort").reset_index(drop=True)

    # n_train_steps = number of steps that still have a labeled target
    first_series = frame[SERIES_COL].iloc[0]
    target_missing = (
        frame.loc[frame[SERIES_COL] == first_series]
        .sort_values(TIME_COL)[TARGET_COL]
        .isna()
        .to_numpy()
    )
    n_train_steps = int(np.argmax(target_missing)) if target_missing.any() else len(target_missing)
